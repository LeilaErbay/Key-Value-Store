...
void kv_delete_db() {
    ...
    shm_unlink(WRITER_SEMAPHORE);
    shm_unlink(READER_SEMAPHORE);
}
...
int main(){
    ...
    for(int k = 0; k < RUN_ITERATIONS; k++){
        ...
        if(pids[__TEST_FORK_NUM__ - 1] > 0){        // parent process
            // Wait for childs to finish up
            temp_flag = kv_store_create(__TEST_SHARED_MEM_NAME__);
            if(temp_flag != 0){
                printf("Error! Init failed in parent\n");
                errors++;
            }
            for(int i = 0; i < __TEST_FORK_NUM__; i++){
                 sem_post_val = sem_post(open_sem_lock);
            }
            for(int i = 0; i < __TEST_FORK_NUM__; i++){
                waitpid(pids[i], NULL, 0);
            }
        }else{                                                      //CHILDREN SHOULD GO HERE?
            open_sem_lock = sem_open(__TEST_SHARED_SEM_NAME__, 0);
            errno = 0;
            sem_wait(open_sem_lock);
            temp_flag = kv_store_create(__TEST_SHARED_MEM_NAME__);
            if(temp_flag != 0){
                printf("Error! Init failed in child \n");
            }
            int i;
            //printf("should start writing");
